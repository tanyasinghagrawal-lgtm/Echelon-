...
async def get_web_app_dashboard_data(user_id: int, bot_username: str = None, time_filter: str = "24h"):
    """
    Mini App Dashboard ke liye stats, clicks, views, active users, revenue,
    aur 5-minute latency time-series return karta hai with strict security.
    """
    ...
    # Agar regular owner kisi aur ke bot ka data mangne ki koshish kare -> 403 Forbidden
    if bot_username and bot_username != "all":
        ...
        if not is_super_admin and clean_target not in user_bot_list:
            return {"error": "Unauthorized access! Aap sirf apne bots ka data dekh sakte hain."}, 403

    # Normal user ke liye "all" ka matlab sirf uske apne bots ka aggregate hona chahiye
    if not is_super_admin and (not bot_username or bot_username == "all"):
        if user_bot_list:
            bot_username = user_bot_list[0]
        else:
            return {"error": "Aapka koi bot register nahi hai."}, 404

    # 2. Time Filter Calculations
    now_ts = int(datetime.utcnow().timestamp())
    time_threshold = {
        "1h": now_ts - 3600,
        "6h": now_ts - (6 * 3600),
        "7d": now_ts - (7 * 86400)
    }.get(time_filter, now_ts - 86400) # Default 24h

    # 3. Latency & Requests Time-Series Graph Data
    latency_series = []
    for b in list(HISTORICAL_5MIN_METRICS):
        if b['epoch'] >= time_threshold:
            if is_super_admin and bot_username == "all":
                reqs = b['total_requests']
            else:
                clean_target = bot_username.split('#')[0]
                reqs = b['bot_requests'].get(clean_target, 0)
                
            latency_series.append({
                "timestamp": b['timestamp'],
                "avg_latency": b['avg_latency_ms'],
                "max_latency": b['max_latency_ms'],
                "p95_latency": b['p95_latency_ms'],
                "requests": reqs,
                "concurrency": b['peak_concurrency']
            })

    # 4. Clicks, Views, Active Users from Valkey
    today_str = datetime.utcnow().strftime('%Y-%m-%d')
    total_clicks = 0
    today_clicks = 0
    today_views = 0
    ...
    target_bots = user_bot_list if (is_super_admin and bot_username == "all") else [bot_username]

    for b_name in target_bots:
        try:
            tot_c = await valkey_client.get(f"stats:{b_name}:clicks:total")
            tod_c = await valkey_client.get(f"stats:{b_name}:clicks:{today_str}")
            tod_v = await valkey_client.get(f"stats:{b_name}:views:{today_str}")
            act_u = await valkey_client.scard(f"stats:{b_name}:active:{today_str}")

            total_clicks += int(tot_c.decode('utf-8') if isinstance(tot_c, bytes) else (tot_c or 0))
            today_clicks += int(tod_c.decode('utf-8') if isinstance(tod_c, bytes) else (tod_c or 0))
            today_views += int(tod_v.decode('utf-8') if isinstance(tod_v, bytes) else (tod_v or 0))
            active_users_today += int(act_u or 0)
        except Exception as ve:
            logger.error(f"Valkey metrics read error for {b_name}: {ve}")

    # 5. Database Stats (Total Users, Premium, Revenue)
    total_users_count = 0
    total_prem_count = 0
    total_links = 0
    total_rev = 0.0
    ...
    if bot_username and bot_username != "all":
        safe_b = DBManager._get_safe_tablename(bot_username, '')
        try:
            u_res = await DBManager.execute_pg_query(f"SELECT COUNT(*) as c FROM {safe_b}users", fetch='one')
            p_res = await DBManager.execute_pg_query(f"SELECT COUNT(*) as c FROM {safe_b}premium", fetch='one')
            f_res = await DBManager.execute_pg_query(f"SELECT COUNT(*) as c FROM {safe_b}files", fetch='one')
            m_res = await DBManager.execute_pg_query(f"SELECT COUNT(*) as c FROM {safe_b}multi_files", fetch='one')
            
            total_users_count = u_res['c'] if u_res else 0
            total_prem_count = p_res['c'] if p_res else 0
            total_links = (f_res['c'] if f_res else 0) + (m_res['c'] if m_res else 0)

            earn_res = await DBManager.execute_pg_query(f"""
                SELECT 
                    COALESCE(SUM(CASE WHEN currency = 'XTR' THEN amount * 1.2 ELSE amount END), 0) as total,
                    COALESCE(SUM(CASE WHEN completion_time >= CURRENT_DATE THEN (CASE WHEN currency = 'XTR' THEN amount * 1.2 ELSE amount END) ELSE 0 END), 0) as today
                FROM {safe_b}successful_transactions;
            """, fetch='one')
            if earn_res:
                total_rev = float(earn_res['total'])
                today_rev = float(earn_res['today'])
        except Exception as e:
            logger.error(f"DB stats fetch error: {e}")

    return {
        "success": True,
        "is_super_admin": is_super_admin,
        "allowed_bots": user_bot_list,
