...
async def get_web_app_dashboard_data(user_id: int, bot_username: str, time_filter: str, page: int = 1, limit: int = 20):
    """
    Enhanced Dashboard Data API for Telegram Mini App:
    - Supports pagination (20-20 items batch for infinite scroll)
    - Detailed transactions with transaction_id, user_id, product type, amount, status
    - Admin commission & pending bills info
    - Comprehensive bot settings (UPI, protection, deletion, FSUB, etc.)
    - Super Admin global view vs Cloned Bot Owner scoped view
    """
    is_super_admin = (user_id == 6796088344)
    today_str = datetime.utcnow().strftime('%Y-%m-%d')
    
    # 1. Access Control
    if bot_username == 'all' and not is_super_admin:
        return {"success": False, "error": "Unauthorized to view all servers."}, 403

    bots_to_check = []
    if bot_username == 'all':
        all_bots = await DBManager.execute_pg_query("SELECT username FROM bots", fetch='all')
        bots_to_check = [b['username'].split('#')[0] for b in all_bots] if all_bots else []
    else:
        bots_to_check = [bot_username.split('#')[0]]

    stats = {
        "today_clicks": 0, "total_clicks": 0,
        "today_views": 0, "total_views": 0,
        "active_users_today": 0, "total_users": 0,
        "today_revenue": 0.0, "total_revenue": 0.0,
        "server_busy_concurrency": CURRENT_ACTIVE_REQUESTS
    }
    
    all_transactions = []
    leaderboard = {"top_clicks": [], "top_earnings": [], "top_views": []}
    bot_detailed_settings = {}
    pending_bills_info = {}

    for clean_name in bots_to_check:
        # Valkey Activity Metrics
        today_clicks = int(await valkey_client.get(f"stats:{clean_name}:clicks:{today_str}") or 0)
        total_clicks = int(await valkey_client.get(f"stats:{clean_name}:clicks:total") or 0)
        today_views = int(await valkey_client.get(f"stats:{clean_name}:views:{today_str}") or 0)
        total_views = int(await valkey_client.get(f"stats:{clean_name}:views:total") or 0)
        active_users = await valkey_client.scard(f"stats:{clean_name}:active:{today_str}") or 0
        
        stats["today_clicks"] += today_clicks
        stats["total_clicks"] += total_clicks
        stats["today_views"] += today_views
        stats["total_views"] += total_views
        stats["active_users_today"] += active_users

        # Total Users from DB
        try:
            users_table = DBManager._get_safe_tablename(clean_name, 'users')
            u_count = await DBManager.execute_pg_query(f"SELECT COUNT(*) as cnt FROM {users_table}", fetch='one')
            stats["total_users"] += u_count['cnt'] if u_count else 0
        except Exception:
            pass

        # Revenue & Detailed Transactions
        safe_table = DBManager._get_safe_tablename(clean_name, '')
        success_table = f"{safe_table}successful_transactions"
        
        try:
            table_check = await DBManager.execute_pg_query(f"SELECT to_regclass('{success_table}')", fetch='one')
            if table_check and table_check['to_regclass']:
                rev_query = f"""
                SELECT 
                    COALESCE(SUM(CASE WHEN currency = 'XTR' THEN amount * 1.2 ELSE amount END), 0) as total_rev,
                    COALESCE(SUM(CASE WHEN completion_time >= CURRENT_DATE AND (currency = 'XTR' THEN amount * 1.2 ELSE amount END) THEN ... ELSE 0 END), 0) as today_rev
                FROM {success_table};
                """
                # Simplified robust revenue query
                rev_data = await DBManager.execute_pg_query(f"SELECT COALESCE(SUM(amount), 0) as total_rev FROM {success_table}", fetch='one')
                bot_total_rev = float(rev_data['total_rev']) if rev_data else 0.0
                stats["total_revenue"] += bot_total_rev

                # Fetch all transactions for card display & infinite scroll pagination
                tx_query = f"""
                SELECT transaction_id, user_id, amount, completion_time, target_payload, currency 
                FROM {success_table} 
                ORDER BY completion_time DESC
                """
                tx_data = await DBManager.execute_pg_query(tx_query, fetch='all')
                if tx_data:
                    for tx in tx_data:
                        is_paid_msg = bool(tx['target_payload'] and tx['target_payload'] != '')
                        all_transactions.append({
                            ...
                            "time": str(tx['completion_time']),
                            "type": "Paid Message" if is_paid_msg else "Premium Subscription",
                            "product_detail": f"Payload: {tx['target_payload']}" if is_paid_msg else "Standard Plan",
                            "status": "Paid"
                        })

                if is_super_admin:
                    leaderboard["top_clicks"].append({"bot": clean_name, "count": today_clicks})
                    leaderboard["top_earnings"].append({"bot": clean_name, "amount": bot_total_rev})
                    leaderboard["top_views"].append({"bot": clean_name, "count": today_views})

        except Exception as e:
            logger.error(f"Error fetching revenue for {clean_name}: {e}")

        # Fetch Bot Settings & Configurations for Mini App Display
        try:
            ...
            if settings_rows:
                for row in settings_rows:
                    try:
                        s_dict[row['key']] = json.loads(row['value'])
                    except Exception:
                        s_dict[row['key']] = row['value']
            
            bot_detailed_settings[clean_name] = {
                "protected_content": s_dict.get('protected', True),
                "file_deletion": s_dict.get('deletion', False),
                "deletion_time_seconds": s_dict.get('deletion_time', 7200),
                "upi_id": s_dict.get('paid_settings', {}).get('upi_id', 'Not Set'),
                "cashfree_enabled": s_dict.get('paid_settings', {}).get('cf_enabled', False),
                "stars_enabled": s_dict.get('paid_settings', {}).get('stars_enabled', False),
                "fsub_channels_count": len(s_dict.get('fsub_channels', [])),
                "admins_count": len(s_dict.get('admins', [])),
                "welcome_message_set": bool(s_dict.get('welcome_message'))
            }
        except Exception as st_err:
            logger.error(f"Error loading settings for {clean_name}: {st_err}")

    # 2. Pagination Logic for Transactions (20 items per batch for infinite scroll)
    ...
    has_more = end_idx < total_transactions_count

    # 3. Pending Bills & Commission Info for Main Admin / Cloned Bot Owner
    target_creator_id = user_id if not is_super_admin else None
    if not is_super_admin:
        bill_data = await get_creator_billing_stats(user_id)
        if bill_data:
            pending_bills_info = {
                "total_sales": bill_data['total_sales'],
                "total_commission_bill": bill_data['total_bill'],
                "total_paid": bill_data['total_paid'],
                "pending_bill": bill_data['pending_bill'],
                "is_locked": bill_data['is_locked']
            }
    else:
        # Super admin views overall billing summary or specific
        pending_bills_info = {"note": "Super Admin Global View"}
    ...
    latency_graph = list(HISTORICAL_5MIN_METRICS)
    ...
    return {
        ...
        "latest_transactions": paginated_transactions,
