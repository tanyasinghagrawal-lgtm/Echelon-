...
class DBManager:
    ...
    async def setup_payment_infrastructure():
        ...
        logger.info("Payment infrastructure tables (active_transactions, id_counter) safaltapoorvak banaye gaye.")
...
# YEH NAYA FUNCTION ADD KAREIN
    ...
    async def handle_clone_bot_start(self):
        ...
        # --- Baaki ka code (payload ke liye) waise hi rahega ---
        if len(payload) == 17:
    ...
    async def perform_delete_bot(self, message, bot_username):
        ...
        suffixes_to_delete = ['files', 'captions', 'multi_files', 'settings', 'users', 'premium']
        deleted_tables = []
        failed_tables = []
    ...
    async def show_paid_settings(self, message, bot_username):
        ...
        # YAHAN BRACKETS KO ESCAPE KIYA GAYA HAI: \( Name: ... \)
        text = (
        ...
        keyboard = [
            [InlineKeyboardButton("✏️ Set/Update Details", callback_data=f"paid_setup_{bot_username}")],
            [InlineKeyboardButton("🤖 Setup AI Verify", callback_data=f"paid_ai_setup_{bot_username}"), ai_toggle_btn],
            [InlineKeyboardButton("❌ Disable Feature", callback_data=f"paid_disable_{bot_username}")],
            [InlineKeyboardButton("⬅️ Back", callback_data=f"bot_settings_{bot_username}")]
        ]
    ...
    async def handle_plan_selection(self, message, days, payload):
        """User ke plan selection ko handle karta hai, transaction banata hai aur QR bhejta hai."""
        settings = await self.get_bot_settings()
        ...
        if not price or not upi_id:
            await message.edit_text("Sorry, this plan is not available. Please try again.")
            return

        # YAHAN SE CHANGE HAI: Hum purana message delete nahi kar rahe hain
        # await message.delete() # <-- YEH LINE HATA DI GAYI HAI
        
        # Hum purane message ko edit karke "Processing..." dikha denge
        try:
            await message.edit_text("Generating your payment QR code...", reply_markup=None)
        except Exception:
            pass # Agar fail ho, to koi baat nahi

        transaction_id = await DBManager.get_next_transaction_id()
        
        # Active transaction DB me entry
        query = """
        ...
        """
        await DBManager.execute_pg_query(query, (
            transaction_id, self.bot_username, admin_id, self.user_id, float(price), days, upi_id
        ))
        ...
        try:
            ...
            caption = (
                ...
                f"**Hinglish:**\n"
                f"1\. Payment karne ke liye kisi bhi UPI app se QR code scan karein\.\n"
                f"2\. Payment safal hone ke baad, neeche 'Upload Screenshot' button par click karein\."
            )
            ...
            # Ab hum plan selection wale message ko delete kar sakte hain
            try:
                await message.delete()
            except Exception:
                pass # Koi baat nahi agar delete na ho

        except Exception as e:
            logger.error(f"QR Code generate/send karte waqt error: {e}")
            await self.bot.send_message(self.chat_id, "Sorry, there was an error creating the payment QR code. Please try again later.")
    async def handle_conv_payment_screenshot(self, state):
        """User dwara bheje gaye screenshot ko handle karta hai, with AI Verification."""
        key = f"{self.bot_username}_{self.user_id}"
        await CACHE_CONVERSATION.delete(key)
        ...
        safe_bot_username = self._escape_markdown(bot_username_from_tx)
        ...
        if ai_enabled and ai_api_key and ai_expected_name:
            ...
            try:
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    # Parse the string inside the text field
                ...
                if is_approved:
                    # ✅ AI APPROVED LOGIC (Directly give premium & Notify Admin)
                    await wait_msg.delete()
                    
                    # Give premium logic directly
                    ...
                    await self.bot.send_message(self.chat_id, f"✅ **AI Approved!** Aapka payment automatically verify ho gaya hai. Aap ab {days} days ke liye premium member hain!", parse_mode=ParseMode.MARKDOWN_V2)
                    
                    # Notify Admin of Auto-Approval
                    admin_text = f"🤖 **AI Auto-Approved Payment**\n\n**Bot:** `@{safe_bot_username}`\n**Tx ID:** `{transaction_id}`\n**Amount:** `₹{tx_data['amount']}`"
                    await self.bot.copy_message(chat_id=admin_id, from_chat_id=self.chat_id, message_id=self.update.message.message_id, caption=admin_text, parse_mode=ParseMode.MARKDOWN_V2)
                    return

                else:
                    # ❌ AI DENIED LOGIC (Notify User, Forward to Admin for Manual Check)
                    await wait_msg.delete()
                    user_deny_msg = f"Sorry, automatic AI ne aapka payment verify nahi kiya.\n**Reason:** {reason}\n\nAapka screenshot ab admin online aakar manually check karenge."
                    await self.bot.send_message(self.chat_id, user_deny_msg)
                    
                    # Add AI Reason to Admin's Caption
                    caption = (
                        f"**New Payment (AI DENIED)**\n\n"
                        f"**Bot:** `@{safe_bot_username}`\n"
                        f"**Tx ID:** `{transaction_id}`\n"
                        f"**Amount:** `₹{tx_data['amount']}`\n"
                        f"**AI Reason:** {reason}\n\n"
                        f"Please check manually and confirm\."
                    )
                    await self.bot.copy_message(chat_id=admin_id, from_chat_id=self.chat_id, message_id=self.update.message.message_id, caption=caption, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN_V2)
                    return
                ...
                # Fallback to normal manual sending below

        # ==========================================
        # MANUAL REVIEW FALLBACK (If AI disabled or failed)
        # ==========================================
        try:
            caption = (
                ...
                f"**Amount:** `₹{tx_data['amount']}`\n"
                f"**Plan:** `{tx_data['plan_duration_days']} Days`\n"
                f"**Transaction ID:** `{transaction_id}`\n\n"
                f"Please check the screenshot and confirm if you have received the payment\."
            )
            ...
            thank_you_text = (
                "Thank you\! Apka screenshot admin ko bhej dia gya hai\.\n\n"
                "Admin jab online ayenge wo apke payment ko verify kar kar apko premium de denge\. "
                "Generally admin jaldi hi verify kar dete hai\."
            )
            await self.bot.send_message(self.chat_id, thank_you_text, parse_mode=ParseMode.MARKDOWN_V2)
        except Exception as e:
            ...
            await self.bot.send_message(self.chat_id, error_text, parse_mode=ParseMode.MARKDOWN_V2)    
    ...
    async def process_payment_confirmation(self, message, transaction_id, is_successful):
        ...
        if is_successful:
            ...
            if clone_bot_instance:
                # Auto Sync call feature ke liye
    ...
    async def reverse_payment(self, admin_message, transaction_id, reverse_to_fail):
        ...
        try:
            ...
            if reverse_to_fail:
                ...
                admin_reply_text = "✅ Transaction reversed. User premium has been CANCELED."
                ...
                admin_reply_text = "✅ Transaction reversed. User has been GRANTED premium."
    ...
    async def get_bot_settings(self, bot_username=None):
        ...
        settings = {
            ...
            # --- PAYMENT FEATURE KE LIYE NAYI SETTINGS ---
            'paid_enabled': False,
            'paid_settings': {'upi_id': '', 'price_7': 0, 'price_28': 0, 'price_90': 0},
            # --- NAYI SETTINGS KHATAM ---
            'unknown_payload_enabled': False,  # <-- NAYA: Unknown payload ki default state RAM ke liye
            'premium_sync_enabled': False      # <-- NAYA: Premium Sync ki default state RAM ke liye
        }
        # --- NAYA CODE KHATAM ---
