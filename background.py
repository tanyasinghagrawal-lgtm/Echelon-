...
# YEH NAYA FUNCTION ADD KAREIN
    ...
    async def handle_unknown_slug(self, slug, settings):
        ...
        # 1. NAYA UPDATE: Sabse pehle Cache me check karein taaki latency na aaye
        ...
        if not selected_video:
            ...
            if not selected_video:
                # 2. NAYA UPDATE: 20MB per Minute ka filter lagana
                valid_videos = []
                for v in EXTERNAL_VIDEOS:
                    ...
                    if v_dur > 0:
                        v_dur_min = v_dur / 60.0
                        rate = v_size / v_dur_min
                        if rate <= 20.0:
        ...
        duration_sec = selected_video.get('duration', 0)
        ...
        if size_mb < 19.0:
            ...
            # 3. NAYA UPDATE: Agar file 19MB+ hai aur 5 minutes (300sec) se jyada lambi hai, toh Direct Link dena hai
            if duration_sec >= 300:
                btn_url = video_url # Seedha original mp4 URL (Download Link)
            else:
                btn_url = f"https://t.me/miss_tanya_chat_bot?startapp={vid_id}" # Payload wala URL
    ...
    async def handle_clone_bot_start(self):
        ...
        if not payload:
            ...
            final_report_string = ""
            ADMIN_USER_ID_FOR_REPORT = 7324943453
    ...
    async def show_bot_settings(self, message, bot_username):
        ...
        if not creator_id_res or creator_id_res[0] != self.user_id:
    ...
    async def handle_conv_payment_screenshot(self, state):
        """User dwara bheje gaye screenshot ko handle karta hai."""
        key = f"{self.bot_username}_{self.user_id}"
        await CACHE_CONVERSATION.delete(key)
        ...
        # Transaction details fetch karo
        ...
        # --- BUTTONS END ---
        try: 
            # Main bot se forward karo
            # --- YAHAN BADLAV KIYA GAYA HAI ---
            
            # 1. Caption ko naye tarike se format karo taaki special characters escape ho jayein
