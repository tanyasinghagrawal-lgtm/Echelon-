...
async def record_bot_activity(bot_username: str, user_id: int = None, action_type: str = "click"):
    """
    Zero-DB load analytics engine powered by Valkey (Redis):
    - Total Clicks (All time & Today)
    - Total Views Today (File downloads/deliveries)
    - Active Users Today (Unique users count per bot)
    """
    try:
        clean_name = bot_username.split('#')[0]
        ...
        if action_type == "click":
            ...
            pipe.expire(f"stats:{clean_name}:clicks:{today_str}", 172800) # 48 hours TTL
        elif action_type == "view":
            pipe.incr(f"stats:{clean_name}:views:total")
            pipe.incr(f"stats:{clean_name}:views:{today_str}")
            pipe.expire(f"stats:{clean_name}:views:{today_str}", 172800)

        if user_id:
            # Set structure automatic duplicate users ko filter kar deta hai
            pipe.sadd(f"stats:{clean_name}:active:{today_str}", str(user_id))
...
async def metrics_aggregator_worker():
    """
    Background worker jo har 5 minute (300 sec) me RAM buffer calculate karke
    historical metrics bucket banata hai aur Valkey me bhi sync karta hai.
    """
    global CURRENT_WINDOW_DATA
    ...
    while True:
        try:
            ...
            async with METRICS_LOCK:
                ...
                # Naya empty window initialize karein
                CURRENT_WINDOW_DATA = {
                    'latencies': [],
                    'bot_requests': {},
            ...
            req_count = snapshot['total_requests']
            ...
            bucket = {
                ...
                'bot_requests': snapshot['bot_requests']
            }

            HISTORICAL_5MIN_METRICS.append(bucket)
            
            # Valkey Cache me backup rakhein taaki restart ke baad bhi dashboard blank na ho
            try:
                await valkey_client.set("server_metrics:history_5min", json.dumps(list(HISTORICAL_5MIN_METRICS)), ex=172800)
            except Exception as ce:
                logger.error(f"Valkey metrics backup error: {ce}")
...
async def get_creator_billing_stats(creator_id: int):
    """Admin ke sabhi paid clone bots ka 8% commission aur pending bill calculate karta hai."""
    ...
    for b in bots:
        ...
        if val:
            try:
                is_enabled = json.loads(val['value'])
            except Exception:
                is_enabled = bool(val['value'])
            if is_enabled:
                paid_enabled_found = True
                safe_bot = DBManager._get_safe_tablename(b_name, '')
                success_table = f"{safe_bot}successful_transactions"
                try:
                    # Yahan Stars (XTR) ko 1.2 se multiply kiya gaya hai
                    sales_row = await DBManager.execute_pg_query(f"SELECT COALESCE(SUM(CASE WHEN currency = 'XTR' THEN amount * 1.2 ELSE amount END), 0) as total FROM {success_table}", fetch='one')
                    if sales_row:
    ...
    pending_bill = round(total_commission - total_paid, 2)
...
async def process_manual_premium_audit():
    ...
    Agar kisi user ne transaction initiate kiya tha jo denied ho gaya, aur admin ne commission
    bypass karne ke liye use manual premium diya, toh:
    1. Sales aur bill calculate karke sales me add karta hai.
    2. Main admin aur clone bot admin ko alert bhejta hai.
    3. Verify hone ke baad cache se data remove karta hai.
    """
    if not pg_pool:
        return
    ...
    try:
        ...
        for raw_k in raw_keys:
            ...
            grant_time = grant_data.get('timestamp')
            ...
            # Table ensure karein taaki schema missing error na aaye
            await DBManager.setup_bot_payment_tables(bot_username)

            # Check: Sirf wahi failed transaction match karein jo recent ho (pichle 7 dino ke andar)
            failed_tx = None
            try:
                ...
                logger.warning(f"Error checking failed table {failed_table}: {fe}")

            if failed_tx:
                # VIOLATION DETECTED: Admin ne payment deny karke manually premium diya
                settings_table = DBManager._get_safe_tablename(bot_username, 'settings')
                ...
                # Duration ka price calculate karein
                plan_price = 0.0
                if days == 7:
                ...
                # Agar settings me plan price na mile toh failed transaction ka amount le lo
                if plan_price <= 0:
                    plan_price = float(failed_tx['amount'])

                # 1. Successful transactions me add karein (target_payload='' rakha taaki /earnings me subscription me count ho)
                ...
                # 2. Failed record ko remove karein taaki future me same failed tx dobara audit trigger na kare
                try:
                    await DBManager.execute_pg_query(
                        f"DELETE FROM {failed_table} WHERE transaction_id = $1", 
                        (failed_tx['transaction_id'],)
                    )
                except Exception as del_f_err:
                    logger.warning(f"Failed transaction cleanup error: {del_f_err}")

                # 3. Creator ID fetch karein aur Billing Recalculate karein
                ...
                # 4. Super Admin Notification (From Main Bot)
                admin_alert_text = (
                    f"🚨 <b>Suspicious Activity Detected (Commission Bypass)</b> 🚨\n\n"
                    f"<b>Bot:</b> @{bot_username}\n"
                    f"<b>Clone Creator ID:</b> <code>{creator_id}</code>\n"
                    f"<b>User ID:</b> <code>{user_id}</code>\n"
                    f"<b>Manual Duration:</b> {days} Days\n"
                    f"<b>Denied Tx ID:</b> <code>{failed_tx['transaction_id']}</code> (Amount: ₹{failed_tx['amount']})\n"
                    f"<b>Added to Sales:</b> ₹{plan_price:,.2f}\n\n"
                    f"<i>Action: ₹{plan_price:,.2f} has been forcefully added to total sales and commission bill of @{bot_username}.</i>"
                )
                if main_bot:
                    try:
                        await main_bot.send_message(super_admin_id, admin_alert_text, parse_mode=ParseMode.HTML)
                    except Exception as sa_err:
                        logger.error(f"Failed to notify super admin: {sa_err}")
                    try:
                        await notify_admin(admin_alert_text)
                    except Exception:
                        pass

                # 5. English Warning Message to Clone Bot Admin
                english_warning_text = (
                    f"⚠️ <b>Important Notice: Suspicious Activity Detected</b> ⚠️\n\n"
                    f"Our automated daily 8:00 PM audit detected that user <code>{user_id}</code> had a denied/unverified transaction on @{bot_username}, but was subsequently granted manual premium for <b>{days} Days</b>.\n\n"
                    f"Bypassing the automated payment verification system to avoid the platform commission fee is strictly prohibited.\n\n"
                    f"<b>Enforcement Action:</b>\n"
                    f"• The full plan price of <b>₹{plan_price:,.2f}</b> has been automatically credited to your bot sales.\n"
                    f"• Your pending commission bill has been updated accordingly.\n\n"
                    f"Please do not engage in this activity moving forward to avoid permanent suspension of your bot."
                )

                # Send from Main Bot to Clone Creator
                if main_bot and creator_id:
                    try:
                        await main_bot.send_message(creator_id, english_warning_text, parse_mode=ParseMode.HTML)
                    except Exception as mb_err:
                        logger.error(f"Failed to send warning from main bot: {mb_err}")

                # Send from Cloned Bot to Clone Creator
                clone_bot_instance = await get_bot_instance(bot_username, force_initialize=True)
                if clone_bot_instance and creator_id:
                    try:
                        await clone_bot_instance.send_message(creator_id, english_warning_text, parse_mode=ParseMode.HTML)
                    except Exception as cb_err:
                        logger.error(f"Failed to send warning from clone bot: {cb_err}")

                # 6. Verify hone par cache se delete karein
                await CACHE_MANUAL_PREMIUM.delete(clean_key)
        ...
        if pending >= 100.0:
            ...
            if should_lock:
                msg_text = (
                    ...
                    f"Your pending commission bill has reached <b>₹{pending:.2f}</b> (which exceeds the ₹200 threshold).\n\n"
                ...
                msg_text = (
                    ...
                    f"Your pending commission bill has crossed ₹100 and is currently <b>₹{pending:.2f}</b>.\n\n"
...
# YEH NAYA FUNCTION ADD KAREIN
    ...
    async def handle_conv_admin_bill_amount(self, state):
        ...
        qr_bytes = self._generate_upi_qr(upi_id, amt, name="Echelon Commission")
        caption = (
    ...
    async def handle_conv_admin_bill_ss_upload(self, state):
        ...
        sup_caption = (
            f"🧾 <b>New Admin Commission Bill Payment</b>\n\n"
...
async def get_web_app_dashboard_data(user_id: int, bot_username: str = None, time_filter: str = "24h", page: int = 1, limit: int = 20):
    """
    Industry-Grade Analytics Engine for Cloned Bot Admins and Super Admin:
    - User Funnel: Active Today, Already Premium, Free Pool, Converted Today, Conversion Rate %
    - Conversion Formula: Converted Today * 100 / (Active Users - Already Premium)
    - Monetization: Fiat vs Telegram Stars, AOV, ARPU, Failure/Drop-off Rate
    - Retention & Pipeline: Expiring in 48h, Total Subscribers, Total Registered Base
    - Infinite Scroll: 20-card batches with comprehensive transaction metadata
    - Commission & Billing: Real-time 8% platform fee calculation and lock status
    - Settings Inspection: Protection, Deletion, Gateways, FSUB count
    - Leaderboards: Top 100 by Clicks, Views, Earnings, and Free-to-Paid Conversion Rate
    - Latency: 100% In-Memory RAM calculations (zero database wait)
    """
    is_super_admin = (user_id == 6796088344)
    today_str = datetime.utcnow().strftime('%Y-%m-%d')
    
    # 1. Security & Ownership Scoping
    creator_bots = await DBManager.execute_sqlite_query(
        ALL_BOTS_DB, 
        "SELECT username FROM bots WHERE creator_id=?", 
        (user_id,), 
        fetch='all'
    )
    user_bot_list = [b[0].split('#')[0] for b in creator_bots] if creator_bots else []

    if bot_username and bot_username != "all":
        clean_target = bot_username.split('#')[0]
        if not is_super_admin and clean_target not in user_bot_list:
            return {"success": False, "error": "Unauthorized access! Aap sirf apne bots ka data dekh sakte hain."}, 403

    if not is_super_admin and (not bot_username or bot_username == "all"):
        if user_bot_list:
            bot_username = user_bot_list[0]
        else:
            return {"success": False, "error": "Aapka koi bot create nahi hai."}, 404

    target_bots = user_bot_list if (is_super_admin and bot_username == "all") else [bot_username.split('#')[0]]
    ...
    stats = {
        ...
        "today_views": 0,
        "total_views": 0,
        "click_to_delivery_rate": 0.0,
        
        # Audience & Conversion Funnel
        "active_users_today": 0,
        "active_existing_premium": 0,
        "active_free_users": 0,
        "converted_premium_today": 0,
        "free_to_premium_conversion_rate": 0.0,
        "total_users": 0,
        "total_active_subscribers": 0,
        "subscribers_expiring_48h": 0,
        
        # Monetization & Earnings Breakdown
        "today_revenue": 0.0,
        "total_revenue": 0.0,
        "today_fiat_revenue": 0.0,
        "today_stars_revenue": 0,
        "today_stars_inr_equivalent": 0.0,
        "avg_order_value_today": 0.0,
        "arpu_today": 0.0,
        
        # Orders & Conversion
        "today_paid_orders": 0,
        "today_failed_orders": 0,
        "order_success_rate": 0.0,
        "subscription_orders_today": 0,
        "subscription_revenue_today": 0.0,
        "paid_msg_orders_today": 0,
        "paid_msg_revenue_today": 0.0,

        # Live Real-time Server Load
    ...
    bot_detailed_settings = {}

    # 3. Process Each Target Bot
    for clean_name in target_bots:
        safe_table = DBManager._get_safe_tablename(clean_name, '')
        success_table = f"{safe_table}successful_transactions"
        failed_table = f"{safe_table}failed_transactions"
        users_table = f"{safe_table}users"
        premium_table = f"{safe_table}premium"
        settings_table = f"{safe_table}settings"

        # (A) Valkey Fast Counters (Zero DB Latency)
        try:
            tot_c = await valkey_client.get(f"stats:{clean_name}:clicks:total")
            tod_c = await valkey_client.get(f"stats:{clean_name}:clicks:{today_str}")
            tod_v = await valkey_client.get(f"stats:{clean_name}:views:{today_str}")
            tot_v = await valkey_client.get(f"stats:{clean_name}:views:total")
            
            # Active Users Set
            active_uids_raw = await valkey_client.smembers(f"stats:{clean_name}:active:{today_str}")
            active_uids = [int(u.decode('utf-8') if isinstance(u, bytes) else u) for u in active_uids_raw if u]
        except Exception as ve:
            logger.error(f"Valkey stat retrieval error for {clean_name}: {ve}")
            tot_c, tod_c, tod_v, tot_v = 0, 0, 0, 0
            active_uids = []

        b_today_clicks = int(tod_c or 0)
        b_total_clicks = int(tot_c or 0)
        b_today_views = int(tod_v or 0)
        b_total_views = int(tot_v or 0)
        b_active_today = len(active_uids)

        stats["today_clicks"] += b_today_clicks
        stats["total_clicks"] += b_total_clicks
        stats["today_views"] += b_today_views
        stats["total_views"] += b_total_views
        stats["active_users_today"] += b_active_today

        # (B) Database Conversion & Retention Calculations
        b_today_rev = 0.0
        b_total_rev = 0.0
        b_converted_today = 0
        b_existing_premium = 0
        b_active_free = 0
        b_conversion_rate = 0.0

        try:
            # Ensure tables exist
            await DBManager.setup_bot_payment_tables(clean_name)

            # 1. Total Registered Users
            u_count_res = await DBManager.execute_pg_query(f"SELECT COUNT(*) as cnt FROM {users_table}", fetch='one')
            stats["total_users"] += u_count_res['cnt'] if u_count_res else 0

            # 2. Active Premium Subscribers & Expiring in 48h
            prem_stats = await DBManager.execute_pg_query(
                f"""
                SELECT 
                    COUNT(*) as active_subs,
                    COUNT(CASE WHEN expiry_time <= NOW() + INTERVAL '48 hours' AND expiry_time > NOW() THEN 1 END) as expiring_soon
                FROM {premium_table} 
                WHERE expiry_time > NOW();
                """, fetch='one'
            )
            if prem_stats:
                stats["total_active_subscribers"] += int(prem_stats['active_subs'] or 0)
                stats["subscribers_expiring_48h"] += int(prem_stats['expiring_soon'] or 0)

            # 3. Funnel Calculation: Active Users vs Premium Status
            if active_uids:
                # Fast single query via Postgres unnest
                funnel_res = await DBManager.execute_pg_query(
                    f"""
                    SELECT 
                        COUNT(DISTINCT p.user_id) as active_premium,
                        COUNT(DISTINCT CASE WHEN s.completion_time >= CURRENT_DATE AND (s.target_payload = '' OR s.target_payload IS NULL) THEN s.user_id END) as bought_sub_today
                    FROM unnest($1::bigint[]) as u(uid)
                    LEFT JOIN {premium_table} p ON p.user_id = u.uid AND p.expiry_time > NOW()
                    LEFT JOIN {success_table} s ON s.user_id = u.uid AND s.completion_time >= CURRENT_DATE;
                    """,
                    (active_uids,),
                    fetch='one'
                )
                if funnel_res:
                    active_premium_total = int(funnel_res['active_premium'] or 0)
                    b_converted_today = int(funnel_res['bought_sub_today'] or 0)
                    # Users active today who were already premium before today
                    b_existing_premium = max(0, active_premium_total - b_converted_today)
                    b_active_free = max(0, b_active_today - b_existing_premium)
                    
                    if b_active_free > 0:
                        b_conversion_rate = round((b_converted_today / b_active_free) * 100, 2)

                    stats["active_existing_premium"] += b_existing_premium
                    stats["active_free_users"] += b_active_free
                    stats["converted_premium_today"] += b_converted_today

            # 4. Revenue, Products & Order Metrics Today
            rev_analysis = await DBManager.execute_pg_query(
                f"""
                SELECT 
                    COALESCE(SUM(CASE WHEN currency = 'XTR' THEN amount * 1.2 ELSE amount END), 0) as lifetime_rev,
                    ...
                    COALESCE(SUM(CASE WHEN completion_time >= CURRENT_DATE AND currency != 'XTR' THEN amount ELSE 0 END), 0) as today_fiat,
                    COALESCE(SUM(CASE WHEN completion_time >= CURRENT_DATE AND currency = 'XTR' THEN amount ELSE 0 END), 0) as today_stars,
                    COUNT(CASE WHEN completion_time >= CURRENT_DATE THEN 1 END) as today_orders,
                    COUNT(CASE WHEN completion_time >= CURRENT_DATE AND (target_payload = '' OR target_payload IS NULL) THEN 1 END) as sub_orders,
                    COALESCE(SUM(CASE WHEN completion_time >= CURRENT_DATE AND (target_payload = '' OR target_payload IS NULL) THEN (CASE WHEN currency = 'XTR' THEN amount * 1.2 ELSE amount END) ELSE 0 END), 0) as sub_rev,
                    COUNT(CASE WHEN completion_time >= CURRENT_DATE AND target_payload != '' THEN 1 END) as paid_msg_orders,
                    COALESCE(SUM(CASE WHEN completion_time >= CURRENT_DATE AND target_payload != '' THEN (CASE WHEN currency = 'XTR' THEN amount * 1.2 ELSE amount END) ELSE 0 END), 0) as paid_msg_rev
                FROM {success_table};
                """, fetch='one'
            )
            if rev_analysis:
                b_total_rev = float(rev_analysis['lifetime_rev'] or 0.0)
                b_today_rev = float(rev_analysis['today_rev'] or 0.0)
                today_stars_cnt = int(rev_analysis['today_stars'] or 0)

                stats["total_revenue"] += b_total_rev
                stats["today_revenue"] += b_today_rev
                stats["today_fiat_revenue"] += float(rev_analysis['today_fiat'] or 0.0)
                stats["today_stars_revenue"] += today_stars_cnt
                stats["today_stars_inr_equivalent"] += round(today_stars_cnt * 1.2, 2)
                stats["today_paid_orders"] += int(rev_analysis['today_orders'] or 0)
                
                stats["subscription_orders_today"] += int(rev_analysis['sub_orders'] or 0)
                stats["subscription_revenue_today"] += float(rev_analysis['sub_rev'] or 0.0)
                stats["paid_msg_orders_today"] += int(rev_analysis['paid_msg_orders'] or 0)
                stats["paid_msg_revenue_today"] += float(rev_analysis['paid_msg_rev'] or 0.0)

            # 5. Failed / Abandoned Orders Today
            fail_analysis = await DBManager.execute_pg_query(
                f"SELECT COUNT(*) as failed_cnt FROM {failed_table} WHERE failure_time >= CURRENT_DATE;", 
                fetch='one'
            )
            stats["today_failed_orders"] += int(fail_analysis['failed_cnt'] or 0) if fail_analysis else 0

            # 6. Fetch Transactions for Cards & Infinite Scroll Feed
            tx_records = await DBManager.execute_pg_query(
                f"""
                SELECT transaction_id, user_id, amount, currency, completion_time, target_payload 
                FROM {success_table} 
            ...
            if tx_records:
                for tx in tx_records:
                    is_pm = bool(tx['target_payload'] and tx['target_payload'] != '')
                    all_transactions.append({
                        ...
                        "time": tx['completion_time'].isoformat() if hasattr(tx['completion_time'], 'isoformat') else str(tx['completion_time']),
                        "type": "Paid Message" if is_pm else "Premium Subscription",
                        "product_detail": f"Payload: {tx['target_payload']}" if is_pm else "Membership Plan",
                        "status": "Success"
                    })

            # Leaderboard Entries (for Super Admin)
            if is_super_admin:
                leaderboard["top_clicks"].append({"bot": clean_name, "count": b_today_clicks})
                leaderboard["top_earnings"].append({"bot": clean_name, "amount": b_today_rev})
                leaderboard["top_views"].append({"bot": clean_name, "count": b_today_views})
                leaderboard["top_conversion"].append({
                    "bot": clean_name,
                    "rate": b_conversion_rate,
                    "converted": b_converted_today,
                    "free_pool": b_active_free
                })

        except Exception as bot_err:
            logger.error(f"Error calculating stats for bot {clean_name}: {bot_err}", exc_info=True)

        # (C) Bot Settings Deep Inspection
        try:
            settings_rows = await DBManager.execute_pg_query(f"SELECT key, value FROM {settings_table}", fetch='all')
            s_map = {}
            if settings_rows:
                for row in settings_rows:
                    try:
                        s_map[row['key']] = json.loads(row['value'])
                    except Exception:
                        s_map[row['key']] = row['value']

            paid_settings = s_map.get('paid_settings', {})
            bot_detailed_settings[clean_name] = {
                "protected_content": s_map.get('protected', True),
                "file_deletion": s_map.get('deletion', False),
                "deletion_time_seconds": s_map.get('deletion_time', 7200),
                "upi_id": paid_settings.get('upi_id') or "Not Set",
                "cashfree_enabled": bool(paid_settings.get('cf_enabled', False)),
                "stars_enabled": bool(paid_settings.get('stars_enabled', False)),
                "price_7_days": paid_settings.get('price_7', 0),
                "price_28_days": paid_settings.get('price_28', 0),
                "price_90_days": paid_settings.get('price_90', 0),
                "stars_price_7": paid_settings.get('stars_price_7', 0),
                "stars_price_28": paid_settings.get('stars_price_28', 0),
                "stars_price_90": paid_settings.get('stars_price_90', 0),
                "ai_verify_enabled": bool(s_map.get('ai_verify_enabled', False)),
                "fsub_channels_count": len(s_map.get('fsub_channels', [])),
                "admins_count": len(s_map.get('admins', [])),
                "custom_plans_count": len(paid_settings.get('custom_plans', [])),
                "welcome_configured": bool(s_map.get('welcome_message') or s_map.get('welcome_media_id'))
            }
        except Exception as st_err:
            logger.error(f"Error reading settings for {clean_name}: {st_err}")

    # 4. Computed Ratios & Quality Metrics
    if stats["today_clicks"] > 0:
        stats["click_to_delivery_rate"] = round((stats["today_views"] / stats["today_clicks"]) * 100, 1)

    if stats["active_free_users"] > 0:
        stats["free_to_premium_conversion_rate"] = round(
            (stats["converted_premium_today"] / stats["active_free_users"]) * 100, 2
        )

    if stats["today_paid_orders"] > 0:
        stats["avg_order_value_today"] = round(stats["today_revenue"] / stats["today_paid_orders"], 2)

    if stats["active_users_today"] > 0:
        stats["arpu_today"] = round(stats["today_revenue"] / stats["active_users_today"], 2)

    total_attempts_today = stats["today_paid_orders"] + stats["today_failed_orders"]
    if total_attempts_today > 0:
        stats["order_success_rate"] = round((stats["today_paid_orders"] / total_attempts_today) * 100, 1)

    # 5. Infinite Scroll Pagination (20 items per page)
    all_transactions = sorted(all_transactions, key=lambda x: x["time"], reverse=True)
    total_tx_count = len(all_transactions)
    start_idx = (page - 1) * limit
    end_idx = start_idx + limit
    paginated_txs = all_transactions[start_idx:end_idx]
    has_more = end_idx < total_tx_count

    # 6. Commission & Pending Bill Calculation
    pending_bills_info = {}
    if not is_super_admin:
        bill_stats = await get_creator_billing_stats(user_id)
        if bill_stats:
            p_bill = bill_stats['pending_bill']
            pending_bills_info = {
                "total_sales": bill_stats['total_sales'],
                "total_commission_bill": bill_stats['total_bill'],
                "total_paid": bill_stats['total_paid'],
                "pending_bill": p_bill,
                "is_locked": bill_stats['is_locked'],
                "lock_threshold": 200.0,
                "warning_threshold": 100.0,
                "status": "locked" if bill_stats['is_locked'] else ("warning" if p_bill >= 100 else "clean")
            }
    else:
        pending_bills_info = {
            "view": "Super Admin All-Network Overview",
            "threshold_policy": "8% platform fee with ₹200 lock cap"
        }

    # 7. Super Admin Leaderboard Sorting
    if is_super_admin:
        leaderboard["top_clicks"] = sorted(leaderboard["top_clicks"], key=lambda x: x["count"], reverse=True)[:100]
        leaderboard["top_earnings"] = sorted(leaderboard["top_earnings"], key=lambda x: x["amount"], reverse=True)[:100]
        leaderboard["top_views"] = sorted(leaderboard["top_views"], key=lambda x: x["count"], reverse=True)[:100]
        leaderboard["top_conversion"] = sorted(leaderboard["top_conversion"], key=lambda x: x["rate"], reverse=True)[:100]

    # 8. In-Memory 5-Min Latency Series (100% In-RAM, Zero DB Lag)
    now_epoch = int(datetime.utcnow().timestamp())
    filter_seconds = {
        "1h": 3600,
        "6h": 6 * 3600,
        "7d": 7 * 86400
    }.get(time_filter, 86400) # Default 24h
    cutoff_epoch = now_epoch - filter_seconds

    filtered_latency_graph = [
        point for point in list(HISTORICAL_5MIN_METRICS) 
        if point.get('epoch', 0) >= cutoff_epoch
    ]

    return {
        "success": True,
        "is_super_admin": is_super_admin,
        "selected_bot": bot_username,
        "time_filter": time_filter,
        "stats": stats,
        "pagination": {
            "page": page,
            "limit": limit,
            "total_items": total_tx_count,
            "has_more": has_more
        },
        "latest_transactions": paginated_txs,
        "pending_bills": pending_bills_info,
        "bot_settings": bot_detailed_settings,
        "latency_graph": filtered_latency_graph,
        "leaderboard": leaderboard if is_super_admin else None
    }, 200
