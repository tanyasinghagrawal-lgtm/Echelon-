...
async def process_manual_premium_audit():
    ...
    Detect karta hai agar:
    1. Kisi user ne QR generate kiya tha par user ne screenshot nahi bheja ya admin ne approve/deny nahi kiya.
    2. Admin ne transaction Deny kiya aur phir manual premium de diya.
    Dono cases me service charge bypass detect karke sales me add karega aur alerts bhejega.
    """
    if not pg_pool:
        return
    ...
    try:
        ...
        for raw_k in raw_keys:
            ...
            matched_tx = None
            tx_source = None  # 'active' ya 'failed'
            tx_time = None

            # 1. Check active_upi_transactions:
            # (QR generate hua tha par screenshot pending raha ya admin ne approve/deny nahi kiya)
            try:
                ...
                logger.warning(f"Error checking active_upi_transactions for audit: {ae}")

            # 2. Check failed_transactions:
            # (Admin dwara explicit Deny kiya gaya transaction)
            try:
                ...
                if failed_tx:
                    ...
                    # Agar dono mile toh jo sabse recent hai use pick karo
            ...
            if matched_tx:
                # VIOLATION DETECTED: QR banne ya Deny hone ke baad manual premium diya gaya
                settings_table = DBManager._get_safe_tablename(bot_username, 'settings')
                ...
                # Duration ka plan price nikaalein
                plan_price = 0.0
                if days == 7:
                ...
                # Agar plan price na mile toh matched transaction ka amount le lo
                if plan_price <= 0:
                    plan_price = float(matched_tx.get('amount', 0.0) or 0.0)

                # 1. Successful transactions me forcefully add karein
                ...
                # 2. Source table se cleanup karein taaki dobara same record detect na ho
                if tx_source == 'active':
                    try:
                        await DBManager.execute_pg_query(
                    ...
                    except Exception as del_a_err:
                        logger.warning(f"Active transaction cleanup error: {del_a_err}")
                ...
                # 3. Creator ID fetch karke Creator ki billing recalculate karein
                ...
                # 4. Super Admin Alert
                status_desc = "Pending/Unresolved QR Transaction (Offline/Ignored)" if tx_source == 'active' else "Denied/Rejected Transaction"
                admin_alert_text = (
                    f"🚨 <b>Suspicious Activity Detected (Service Charge Bypass)</b> 🚨\n\n"
                    f"<b>Bot:</b> @{bot_username}\n"
                    f"<b>Clone Creator ID:</b> <code>{creator_id}</code>\n"
                    f"<b>User ID:</b> <code>{user_id}</code>\n"
                    f"<b>Manual Duration:</b> {days} Days\n"
                    f"<b>Detected Status:</b> {status_desc}\n"
                    f"<b>Tx ID:</b> <code>{matched_tx['transaction_id']}</code> (Amount: ₹{matched_tx['amount']})\n"
                    f"<b>Added to Sales:</b> ₹{plan_price:,.2f}\n\n"
                    f"<i>Action: ₹{plan_price:,.2f} has been forcefully added to total sales and service charge bill of @{bot_username}.</i>"
                )
                if main_bot:
                    try:
                ...
                # 5. Warning Notice to Clone Bot Admin
                english_warning_text = (
                    f"⚠️ <b>Important Notice: Suspicious Activity Detected</b> ⚠️\n\n"
                    f"Our automated daily 8:00 PM audit detected that user <code>{user_id}</code> had an active or unverified payment transaction (Tx ID: <code>{matched_tx['transaction_id']}</code>) on @{bot_username}, but was subsequently granted manual premium for <b>{days} Days</b>.\n\n"
                    f"Bypassing the automated payment verification system (including accepting offline payments after QR generation or granting manual access to pending transactions) to avoid the platform service charge fee is strictly prohibited.\n\n"
                    f"<b>Enforcement Action:</b>\n"
                    f"• The full plan price of <b>₹{plan_price:,.2f}</b> has been automatically credited to your bot sales.\n"
                    f"• Your pending service charge bill has been updated accordingly.\n\n"
                ...
                # 6. Audit complete hone par is entry ko cache se delete karein
                await CACHE_MANUAL_PREMIUM.delete(clean_key)
