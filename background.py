...
# --- REAL-TIME LATENCY & SERVER LOAD TRACKER (VALKEY RAM ENGINE) ---
# --- REAL-TIME LATENCY & SERVER LOAD TRACKER (VALKEY RAM ENGINE) ---
async def record_server_metrics(bot_username: str, latency_ms: float, active_concurrency: int, user_id: int = None, is_view: bool = False):
    """
    Latency, Server Concurrency, Total Clicks, Views, aur Active Users ko 
    Valkey ke 5-minute, hourly aur daily RAM buckets me save karta hai.
    """
    try:
        now_ts = int(time.time())
        bucket_5m = (now_ts // 300) * 300       # 5-minute bucket
        bucket_1h = (now_ts // 3600) * 3600     # 1-hour bucket
        today_date = datetime.utcnow().strftime('%Y-%m-%d')
        clean_bot = bot_username.replace("@", "").split('#')[0]

        pipe = valkey_client.pipeline()

        # 1. Global Server 5-Min Bucket (TTL: 48 Hours)
        key_5m = f"metrics:5m:{bucket_5m}"
        pipe.hincrby(key_5m, "req_count", 1)
        pipe.hincrbyfloat(key_5m, "total_latency", round(latency_ms, 2))
        pipe.hsetnx(key_5m, "timestamp", bucket_5m)
        pipe.expire(key_5m, 172800)

        # 2. Global Server 1-Hour Bucket (TTL: 30 Days)
        key_1h = f"metrics:1h:{bucket_1h}"
        pipe.hincrby(key_1h, "req_count", 1)
        pipe.hincrbyfloat(key_1h, "total_latency", round(latency_ms, 2))
        pipe.hsetnx(key_1h, "timestamp", bucket_1h)
        pipe.expire(key_1h, 2592000)

        # 3. Peak Server Concurrency Tracking
        key_concur = f"metrics:concurrency:5m:{bucket_5m}"
        pipe.lpush(key_concur, active_concurrency)
        pipe.ltrim(key_concur, 0, 50)
        pipe.expire(key_concur, 172800)

        # 4. Bot-Wise 5-Min Time-Series Bucket (For Bot Graph Filters)
        key_bot_5m = f"metrics:bot:{clean_bot}:5m:{bucket_5m}"
        pipe.hincrby(key_bot_5m, "clicks", 1)
        if is_view:
            pipe.hincrby(key_bot_5m, "views", 1)
        pipe.hincrbyfloat(key_bot_5m, "total_latency", round(latency_ms, 2))
        pipe.expire(key_bot_5m, 172800)

        # 5. Bot-Wise 1-Hour Time-Series Bucket (For 24h & 7d Graphs)
        key_bot_1h = f"metrics:bot:{clean_bot}:1h:{bucket_1h}"
        pipe.hincrby(key_bot_1h, "clicks", 1)
        if is_view:
            pipe.hincrby(key_bot_1h, "views", 1)
        pipe.hincrbyfloat(key_bot_1h, "total_latency", round(latency_ms, 2))
        pipe.expire(key_bot_1h, 2592000)

        # 6. Bot-Wise Daily Totals (Today clicks, views, latency)
        key_bot_today = f"metrics:bot:{clean_bot}:daily:{today_date}"
        pipe.hincrby(key_bot_today, "clicks", 1)
        if is_view:
            pipe.hincrby(key_bot_today, "views", 1)
        pipe.hincrbyfloat(key_bot_today, "total_latency", round(latency_ms, 2))
        pipe.expire(key_bot_today, 1296000)

        # 7. Total Active Users Today (Ultra-light Valkey Set, TTL: 48h)
        if user_id:
            key_active_users = f"metrics:bot:{clean_bot}:active:{today_date}"
            pipe.sadd(key_active_users, user_id)
            pipe.expire(key_active_users, 172800)

        # 8. Push to Index Sets for Quick Querying
        pipe.zadd("metrics:index:5m", {str(bucket_5m): bucket_5m})
        pipe.zadd("metrics:index:1h", {str(bucket_1h): bucket_1h})
        pipe.zadd(f"metrics:bot:{clean_bot}:index:5m", {str(bucket_5m): bucket_5m})
        pipe.zadd(f"metrics:bot:{clean_bot}:index:1h", {str(bucket_1h): bucket_1h})
...
# YEH NAYA FUNCTION ADD KAREIN
    ...
    async def process(self):
        ...
        # Concurrency measure: Semaphore limit (1260) - available permits = currently busy workers
        current_busy_workers = 1260 - CONCURRENCY_SEMAPHORE._value
        ...
        async with CONCURRENCY_SEMAPHORE:
            try:
                ...
                # Execution complete hone par exact delivery & processing latency log karein
                elapsed_latency_ms = (time.time() - req_start_time) * 1000
                # Check agar link payload start command tha (View count)
                is_payload_view = bool(
                    self.update and self.update.message and 
                    self.update.message.text and 
                    self.update.message.text.startswith('/start') and 
                    len(self.update.message.text.split()) > 1
                )
                asyncio.create_task(
                    record_server_metrics(
                        self.bot_username, 
                        elapsed_latency_ms, 
                        current_busy_workers,
                        user_id=self.user_id,
                        is_view=is_payload_view
                    )
                )    
