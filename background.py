...
async def init_postgresql_pool():
    ...
    for attempt in range(max_retries):
        try:
            # Aiven URL (DSN) ka use karke connection pool banayein (Max 20 & 5 Min Kill Timer)
...
async def process_scheduled_deletions():
    ...
    Jobs claim karke DB connection turant release kar deta hai taaki network calls ke dauran pool block na ho.
    """
    if not pg_pool:
        return
    ...
    # Step 1: Chhote context me connection acquire karo aur turant release karo
    try:
        async with pg_pool.acquire() as conn:
        ...
        logger.error(f"Deletion processor claim error: {e}")
        return

    # Step 2: Connection release ho chuka hai, ab Telegram API calls safe execute hongi
    for job in jobs_to_process:
        try:
            ...
            await DBManager.execute_pg_query("DELETE FROM scheduled_deletions WHERE id = $1", (job['id'],))
            ...
            if "message to delete not found" in error_str or "message can't be deleted" in error_str:
                ...
                new_delete_at = datetime.utcnow() + timedelta(seconds=retry_after_seconds + 5)
                await DBManager.execute_pg_query(
                    """
    ...
    for b in bots:
        ...
        if val:
            ...
            if is_enabled:
                ...
                try:
                    sales_row = await DBManager.execute_pg_query(f"SELECT COALESCE(SUM(amount), 0) as total FROM {success_table}", fetch='one')
                    if sales_row:
...
# YEH NAYA FUNCTION ADD KAREIN
    ...
    async def handle_main_bot_start(self):
        ...
        if payload == "pay":
            ...
            await self.initiate_conversation('admin_bill_amount', bill_msg)
            return
       ...
       # --- 15-CHARACTERS PAID MESSAGE CHECK ---
        if payload and len(payload) == 15:
            ...
            if msg_data:
                ...
                # 2. Agar access nahi hai (naya buyer hai) tab check karo bot locked hai ya nahi
                creator_id = settings.get('creator_id')
    ...
    async def show_earnings_overview(self, message, bot_username):
        ...
        SELECT 
            ...
            COALESCE(SUM(CASE WHEN completion_time >= NOW() - INTERVAL '30 days' THEN amount ELSE 0 END), 0) as rev_30d,
            COUNT(CASE WHEN completion_time >= NOW() - INTERVAL '30 days' THEN 1 END) as orders_30d,
            COALESCE(SUM(CASE WHEN completion_time >= NOW() - INTERVAL '7 days' THEN amount ELSE 0 END), 0) as rev_7d,
            COUNT(CASE WHEN completion_time >= NOW() - INTERVAL '7 days' THEN 1 END) as orders_7d,
            COALESCE(SUM(CASE WHEN completion_time >= CURRENT_DATE THEN amount ELSE 0 END), 0) as rev_today,
            COUNT(CASE WHEN completion_time >= CURRENT_DATE THEN 1 END) as orders_today,
            COALESCE(SUM(CASE WHEN target_payload != '' THEN amount ELSE 0 END), 0) as paid_msg_rev,
            COUNT(CASE WHEN target_payload != '' THEN 1 END) as paid_msg_orders,
            COALESCE(SUM(CASE WHEN target_payload = '' OR target_payload IS NULL THEN amount ELSE 0 END), 0) as sub_rev,
    ...
    async def show_earnings_daily(self, message, bot_username, days: int):
        ...
        if not records:
            ...
            text += f"<b>Total:</b> ₹{total_period:,.2f} across {total_orders} orders\n\n"
            for r in records:
                d_str = r['dt'].strftime('%Y-%m-%d')
                text += f"• <b>{d_str}:</b> ₹{float(r['daily_sum']):,.2f} ({r['daily_count']} orders)\n"
    ...
    async def show_paid_settings(self, message, bot_username):
        ...
        # Check if default 3 plans (7, 28, 90 days) are already set
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("✏️ Setup UPI & Default Prices", callback_data=f"paid_setup_{bot_username}")],
    ...
    async def handle_conv_earnings_custom_date(self, state):
        ...
        query = f"""
        SELECT 
            COALESCE(SUM(amount), 0) as range_rev,
    ...
    async def reverse_payment(self, admin_message, transaction_id, reverse_to_fail):
        ...
        try:
            ...
            move_query = f"""
            INSERT INTO {target_table_name} (transaction_id, user_id, amount, plan_duration_days, {target_time_col}, target_payload, sent_message_id)
            VALUES ($1, $2, $3, $4, NOW(), $5, $6)
            """
            await DBManager.execute_pg_query(move_query, (transaction_id, user_id, amount, days, target_payload or '', sent_msg_id))
    ...
    async def get_bot_settings(self, bot_username=None):
        ...
        settings = {
            ...
            'paid_enabled': False,
            'paid_settings': {'upi_id': '', 'price_7': 0, 'price_28': 0, 'price_90': 0, 'custom_plans': []},
