...
class DBManager:
    ...
    async def setup_payment_infrastructure():
        ...
        # Universal Scammers / Fraud Users Table
        fraud_table_query = """
        CREATE TABLE IF NOT EXISTS global_fraud_users (
            user_id BIGINT PRIMARY KEY,
            bot_username TEXT,
            reason TEXT,
        ...
        """
        await DBManager.execute_pg_query(fraud_table_query)
...
# YEH NAYA FUNCTION ADD KAREIN
    ...
    async def handle_conv_payment_screenshot(self, state):
        ...
        if ai_enabled and ai_api_key and ai_expected_name:
            # Check if user is in Universal Scammer / Fraud list
            is_blacklisted = False
            try:
                blacklisted_record = await DBManager.execute_pg_query(
                    "SELECT 1 FROM global_fraud_users WHERE user_id = $1", 
                    (self.user_id,), 
                    fetch='one'
                )
                ...
                logger.error(f"Error checking global fraud table: {e}")

            if is_blacklisted:
                # Scammer detected: Skip AI Verification
                warning_text = "⚠️ According to your recent credibility, your screenshot will be verified manually by admin."
                await self.bot.send_message(self.chat_id, warning_text)
            else:
                ...
                if is_approved:
                    ...
                    move_query = f"INSERT INTO {safe_bot_table}successful_transactions (transaction_id, user_id, amount, plan_duration_days, completion_time, target_payload) VALUES ($1, $2, $3, $4, NOW(), $5)"
                    await DBManager.execute_pg_query(move_query, (transaction_id, self.user_id, tx_data['amount'], days, target_payload or ''))                    
    ...
    async def reverse_payment(self, admin_message, transaction_id, reverse_to_fail):
        ...
        try:
            ...
            if reverse_to_fail:
                # 1. User ko Universal Fraud Table me blacklist karo
                try:
                    await DBManager.execute_pg_query(
                        """
                        INSERT INTO global_fraud_users (user_id, bot_username, reason) 
                        VALUES ($1, $2, $3) 
                        ON CONFLICT (user_id) DO NOTHING
                        """,
                        (user_id, bot_username, 'Payment cancelled/reversed by admin due to invalid screenshot')
                    )
                except Exception as fraud_err:
                    logger.error(f"Failed to add user {user_id} to global fraud list: {fraud_err}")

                # 2. Paid Message ya Premium Access revoke karo
                if target_payload:
                    paid_access_table = DBManager._get_safe_tablename(bot_username, 'paid_msg_access')
                    await DBManager.execute_pg_query(
                    ...
                    user_notify_message = "⚠️ Aapka paid message payment verification admin dwara cancel kar diya gaya hai. Access revoke ho chuka hai."
                    admin_reply_text = "✅ Transaction reversed. Paid message access REVOKED & User blacklisted globally."
                else:
                    await DBManager.execute_pg_query(f"DELETE FROM {premium_table} WHERE user_id=$1", (user_id,))
                    ...
                    admin_reply_text = "✅ Transaction reversed. User premium CANCELED & User blacklisted globally."
