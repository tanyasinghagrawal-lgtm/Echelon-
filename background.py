...
async def run_cache_cleanup_and_ram_monitor():
    """
    Ek background task jo RAM usage aur Server Health ko monitor karke 
    admin ko regular reports bhejta hai. (Valkey native TTL use karta hai, isliye manual delete ki zarurat nahi).
    """
    logger.info("Server Health & RAM Monitor service shuru ho gayi hai.")
    ...
    while True:
        try:
            ...
            if time_since_last_report >= REPORT_INTERVAL_SECONDS:
                ...
                report_message = (
                    f"📊 **Server Health Report (Valkey Active)** 📊\n\n"
                    ...
                    f"⚡ **Cache Engine:** `Aiven Valkey (Auto TTL)`\n"
                    f"🤖 **Bot Engine:** `Connection Pooling Active`"
                )
            ...
            logger.error(f"Server Health Monitor task mein error aaya: {e}", exc_info=True)
...
async def get_bot_instance(bot_username, force_initialize=None):
    ...
    # 1. Cache me pehle se initialized bot maujood hai toh wahi return karein
    async with BOT_INSTANCES_LOCK:
        if clean_name in BOT_INSTANCES:
    ...
    # 3. Connection Pool bana kar ek hi baar initialize karein
    ...
    # 4. Global pool me save karein taaki bar-bar new object na bane
    async with BOT_INSTANCES_LOCK:
