...
async def get_web_app_dashboard_data(user_id: int, bot_username: str = None, time_filter: str = "24h", page: int = 1, limit: int = 20):
    ...
    Supports time filters ('1h', '6h', '24h', '7d'), real-time bill calculations with advance credit,
    super admin 'all' network aggregation, conversion funnels, and paginated transaction feeds.
    """
    if not pg_pool:
    ...
    # 2. Aggregating Activity Stats (Clicks, Views, Users, Revenue)
    today_str = datetime.utcnow().strftime('%Y-%m-%d')
    total_clicks = 0
    today_clicks = 0
    total_views = 0
    ...
    # Funnel Metrics
    converted_today_count = 0

    for b in target_bots:
        # Clicks & Views from Valkey
        try:
            c_tot = await valkey_client.get(f"stats:{b}:clicks:total")
            c_tdy = await valkey_client.get(f"stats:{b}:clicks:{today_str}")
            v_tot = await valkey_client.get(f"stats:{b}:views:total")
            v_tdy = await valkey_client.get(f"stats:{b}:views:{today_str}")

            total_clicks += int(c_tot or 0)
            today_clicks += int(c_tdy or 0)
            total_views += int(v_tot or 0)
            today_views += int(v_tdy or 0)

            uids = await valkey_client.smembers(f"stats:{b}:active:{today_str}")
            if uids:
                active_users_set.update(uids)
        except Exception:
            pass
        ...
        # Revenue Queries with Time Filter
        try:
            rev_q = f"""
            SELECT 
                ...
                COUNT(CASE WHEN completion_time >= CURRENT_DATE AND (target_payload = '' OR target_payload IS NULL) THEN 1 END) as conv_today
            FROM {success_table};
            """
            ...
            if rq:
                ...
                converted_today_count += int(rq['conv_today'])
        except Exception:
            pass

    active_users_today = len(active_users_set)
    aov = round(period_rev / total_orders_count, 2) if total_orders_count > 0 else 0.0
    arpu = round(period_rev / active_users_today, 2) if active_users_today > 0 else 0.0

    # Conversion Funnel Math
    active_existing_prem = min(active_users_today, total_active_subs)
    active_free_users = max(0, active_users_today - active_existing_prem)
    cr_rate = round((converted_today_count / active_free_users * 100), 1) if active_free_users > 0 else 0.0

    # 3. Bot Settings Configuration Map
    bot_settings_map = {}
    for b in target_bots[:15]:  # Limit to 15 sample bots
        try:
    ...
    # 4. Pending Bills Calculation (with Real-Time Advance Credit)
    pending_bills_data = {}
    ...
    # 6. Network Leaderboard (Super Admin Only)
    ...
    if is_super_admin:
        ...
        for b in lb_bots:
            ...
            try:
                e_row = await DBManager.execute_pg_query(
                    f"SELECT COALESCE(SUM(CASE WHEN currency = 'XTR' THEN amount * 1.2 ELSE amount END), 0) as amt, COUNT(*) as cnt FROM {safe_b}successful_transactions WHERE completion_time >= CURRENT_DATE",
                    fetch='one'
                )
        ...
        leaderboard_data = {
            ...
            "top_conversion": top_earnings[:20]  # Fallback to high performers
        }
    ...
    response_payload = {
        ...
        "stats": {
            ...
            "converted_premium_today": converted_today_count,
