    ...
    async def handle_clone_bot_start(self):
       ...
       # --- 15-CHARACTERS PAID MESSAGE CHECK ---
        ...
        if not is_premium:
            ...
            # Agar admin ne free limit set ki hai (1 se 100)
            if free_limit > 0:
                consumed = await self.consume_free_limit(free_limit)
                if consumed:
                    free_limit_used = True
                else:
                    # Free limit khatam! User ko premium buy karne ko bolein
                    if settings.get('paid_enabled', False):
                        await self.bot.send_message(
                            self.chat_id,
                            "⚠️ <b>Daily Free Limit Exhausted!</b>\n\n"
                            "Aapki aaj ki free access limit khatam ho chuki hai. Aage files access karne ke liye Premium purchase karein ya kal subah 5:00 AM IST tak intezar karein.",
                            parse_mode=ParseMode.HTML
                        )
                        await self.show_payment_plans(self.update.message, payload)
                        return

        # Agar user premium nahi hai aur free limit bhi use nahi hua (default bots / ad-shortener mode)
        if not is_premium and not free_limit_used:
