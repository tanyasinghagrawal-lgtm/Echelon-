...
async def api_dashboard_data(request: Request):
    """
    Returns time-series latency, RPM, users, views, clicks, and earnings stats.
    Protected by Telegram initData HMAC-SHA256 signature verification.
    """
    try:
        body = await request.json()
        init_data = body.get("initData", "")
        ...
        # Super Admin ke alawa kisi ko direct "all" query karne ki permission nahi hai
        if user_id != 6796088344 and (not bot_username or bot_username == "all"):
            creator_bots = await DBManager.execute_sqlite_query(
                ALL_BOTS_DB, "SELECT username FROM bots WHERE creator_id=?", (user_id,), fetch='all'
            )
